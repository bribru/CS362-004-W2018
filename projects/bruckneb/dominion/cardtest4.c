...
* 2. 1 card should come from his own draw pile.
* 3. Current player should gain 2 actions.
...
#define TESTCARDVAL village

int main() {
	// default values for various game elements
	int newCards = 0;
	int usedActions = 1;
	int newActions = 0;
	int playedCards = 1;
	...
	// Village gives +1 card
	newCards = 1;
	// Village uses 1 action
	usedActions = 1;
	...
	printf("    numActions = %d, expected %d\n", testG.numActions, G.numActions - usedActions + newActions);
	...
	*/
	...
	assert(testG.numActions == G.numActions - usedActions + newActions);
